...
wh.w=options.width;wh.h=options.height;container.width(options.width);container.height(options.height);container.css("z-index",highestZindex());if($.browser.msie){container.css({paddingRight:1,paddingBottom:1});}
if(options.x>=0||options.y>=0)
{var scrollPos=getBrowserScrollXY();if(options.x>=0){var pLeft=0;if(caller!=null){var cpos=getParentPanelStartPos(caller);pLeft=options.x+cpos.left;}else{pLeft=options.x+scrollPos.left;}
container.css("left",pLeft);}else{alignHorizontalCenter();}
if(options.y>=0){var pTop=0;if(caller!=null){var cpos=getParentPanelStartPos(caller);console.log('cpos:',cpos);pTop=options.y+cpos.top;}else{pTop=options.y+scrollPos.top;console.log('scrollPos:',scrollPos);}
console.log(pTop);container.css("top",pTop);}
else{alignVerticalCenter();}}
else{console.log("Center");alignCenter();}
var currPos=container.position();console.log(this,this.getCaller());console.log(options);container.css({left:parseInt(currPos.left,10),top:Math.abs(parseInt(currPos.top,10))});if(options.icon=="auto"){options.icon=_prepareFaviconUrl();}
...
function showOverlay(){var overlay=$("#window_overlay");if(overlay.get(0)==null){$("body").append("<div id='window_overlay'>&nbsp;</div>");overlay=$("#window_overlay");overlay.css('z-index',highestZindex());}
if(options.showModal){overlay.css({opacity:options.modalOpacity});}else{overlay.css({opacity:0});}
overlay.show();}
function highestZindex()
{var i=1;$("li,ul,div:visible").each(function(){var v=parseInt($(this).css("z-index"))||0;if(v>i)i=v;});return i+2;}
function hideOverlay(bForce){if(options.showModal==false||bForce){$("#window_overlay").hide();}}
function transferToFixed(){var currPos=container.offset();var scrollPos=getBrowserScrollXY();console.log({position:"fixed",left:currPos.left-scrollPos.left,top:currPos.top-scrollPos.top});container.css({position:"fixed",left:currPos.left-scrollPos.left,top:currPos.top-scrollPos.top});}
function transferToAbsolute(){var currPos=container.offset();console.log({position:"absolute",left:currPos.left,top:currPos.top});container.css({position:"absolute",left:currPos.left,top:currPos.top});}
...
function alignCenter(){var pLeft=0,pTop=0;if(caller!=null){console.log("caller");var cpos=getParentPanelStartPos(caller);cpos.top=caller.scrollTop();console.log(cpos.top,cpos.top+(caller.height()-container.height())/2,cpos.top,caller.height(),container.height());pLeft=cpos.left+(caller.width()-container.width())/2;pTop=cpos.top+(caller.height()-container.height())/2;}
else
{var scrollPos=getBrowserScrollXY();var screenWH=getBrowserScreenWH();pLeft=scrollPos.left+(screenWH.width-container.width())/2;pTop=scrollPos.top+(screenWH.height-container.height())/2;};if(options.createRandomOffset.x>0){pLeft+=((Math.random()-0.5)*options.createRandomOffset.x);}
...
container.css({top:pTop});console.log('alignVerticalCenter',pTop);}
function select(){selected=true;if(maximized==false){container.css('z-index',highestZindex()+2);if(options.selectedHeaderClass){header.addClass(options.selectedHeaderClass);}}
if($.isFunction(options.onSelect)){options.onSelect();}}
function unselect(){selected=false;if(maximized==false){container.css('z-index',highestZindex());if(options.selectedHeaderClass){header.removeClass(options.selectedHeaderClass);}}
...
container.addClass('no-resizable');var scrollPos=getBrowserScrollXY();var screenWH=getBrowserScreenWH();if(caller!=null){var cpos=getParentPanelStartPos(caller);targetCssStyle={left:cpos.left,top:cpos.top,width:caller.width()-3,height:caller.height()-3,opacity:1};}else{targetCssStyle={left:scrollPos.left,top:scrollPos.top,width:screenWH.width,height:screenWH.height,opacity:1};}
...
headerFuncPanel.hide();hideContent();checkMinWindowSize(caller,true);_adjustMinimizedPos(false,function(){container.css('z-index',highestZindex());header.children("div.window_title_text").width("96%");header.attr("title",options.title);header.removeClass('window_header_normal');header.removeClass('window_header_maximize');header.addClass('window_header_minimize');if(setting.dock=='left'||setting.dock=='right'){header.addClass('window_header_minimize_vertical');}
...
var rpos=null;var rwh=null;var zIndex=highestZindex()+2;if(minimized){rpos=orgPos;rwh=orgWh;if(caller==null){transferToAbsolute();}
...
zIndex=highestZindex()+3;container.css('z-index',zIndex);}else{header.addClass('window_header_normal');container.removeClass('no-resizable');}}else if(maximized){maximized=false;rpos=pos;rwh=wh;header.removeClass('window_header_maximize');header.addClass('window_header_normal');container.removeClass('no-resizable');}
...
function close(quiet){if(!quiet&&$.isFunction(options.onClose)){if(options.onClose(_this))return;}
destroy();}
function destroy(){redirectCheck=false;if(maximized){showBrowserScrollbar();}
popWindow(_this);container.remove();hideOverlay(true);}
function show(){if(options.showModal){showOverlay();}
container.show();}
function hide(quiet){if(!quiet&&$.isFunction(options.onHide)){if(options.onHide(_this))return;}
container.hide();hideOverlay(true);log(hide);}
