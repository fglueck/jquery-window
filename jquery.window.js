...
$.Window = (function()  {
	...
	// static constants
	var VERSION = "5.04";           // the version of current plugin
	...
	var OPACITY_MINIMIZED = 0.3;    // css opacity while window minimized or doing animation
...
/**
 * @todo only if window scroll, but if div-scroll ... 
 */
	...
	function constructor(caller, options) {
		...
		function initialize(instance) {
			...
			container.css("z-index", highestZindex()); /// @todo hier evtl. options.z da INIT
			...
			console.log(this, this.getCaller());
			console.log(options);
			container.css({
				left: parseInt(currPos.left,10),
				top:  Math.abs(parseInt(currPos.top,10)) // @todo hier nicht gut
			});
		...
		function showOverlay() {
			...
			if( overlay.get(0) == null ) {
				...
				overlay.css('z-index', highestZindex()); // options.z+1
			}
		...
		{
		    var i = 1;
		    $("li,ul,div:visible").each(function() {
		      var v = parseInt($(this).css("z-index"))||0;
		      if(v>i) i = v;
		    });
		...
		function alignCenter() {
			...
			if( caller != null ) {
			  console.log("caller");
				var cpos = getParentPanelStartPos(caller);
				cpos.top = caller.scrollTop();
				console.log(cpos.top, cpos.top + (caller.height() - container.height()) / 2, cpos.top ,caller.height(), container.height());
				pLeft = cpos.left + (caller.width() - container.width()) / 2;
				pTop  = cpos.top + (caller.height() - container.height()) / 2; /// @todo hier muss die höhe des sichtbaren Bereichs ermittelt werden
			}
			else 
			{			
				var scrollPos = getBrowserScrollXY();
			...
			/// @todo Fehler der Position könnte hier sein
			container.css({
				left: pLeft,
				top:  pTop
			});
		...
		function alignVerticalCenter() {
			...
			console.log('alignVerticalCenter', pTop); /// @todo hier vielleicht auch
		}
		
		function select() {
			selected = true;
			if( maximized == false ) {
				container.css('z-index', highestZindex()+2);
		...
		function unselect() {
			selected = false;
			if( maximized == false ) {
				container.css('z-index', highestZindex());
				if( options.selectedHeaderClass ) {
		...
		function maximize(bImmediately, bNoSaveDisplay) {
			...
			if( caller != null ) {
				...
				targetCssStyle = {
					left: cpos.left,
					top: cpos.top,
					width: caller.width()-3,
					height: caller.height()-3,
					opacity: 1
				};
			} else {
				targetCssStyle = {
			...
			if( bImmediately ) {
				...
				container.css('z-index', highestZindex()+3);
			}
        ...
        if(options.onClose(_this)) return;
			}
			destroy();
		}
      ...
      if( options.showModal ) {
		...
		function hide(quiet) {
		  if( !quiet && $.isFunction(options.onHide) ) {
        if(options.onHide(_this)) return;
      }
			container.hide();
			hideOverlay(true);
			log(hide); 
